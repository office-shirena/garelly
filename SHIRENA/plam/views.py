...
from .models import Pallets, SendPlan, Company,CompanyUser, Pallettype,Message,CompanyRelated,ZaikoTable,Image,UserImage
...
def home(request):
    ...
    if(request.method == 'POST'):
        ...
        maxid = Image.objects.filter(companyCode=companycode).count()
        
        print(maxid)
        for num in range(1,int(maxid)+1):
            quantity = request.POST[str(num)]   
            id = request.POST[str(num) +'id']
            imageno = request.POST[str(num) +'image']
            title =  request.POST[str(num) + 'title']
            print(quantity)
            print(str(id))
            print(imageno)
            print(title)
            #title=Image.objects.filter(companyCode=companycode).values_list('title',flat=True).get(id=num)
            #print(title)
            savedata = UserImage.objects.create(
        ...
        key=UserImage.objects.filter(username = username)
        length = len(key)
            #usercount = User.objects.raw('SELECT * FROM auth_user WHERE email = %s', email)
        print(str(length))
            
        if length == 0 :   
            images = Image.objects.filter(companyCode=companycode)
            ...
            images = UserImage.objects.filter(username=username)
            context = {'images':images,
                       'username':username,
                       'companycode':companycode,
                       'companyname':companyname,
...
def confirm(request):
    ...
    companyname = Company.objects.get(companyCode=companycode)
    if(request.method == 'POST'):
        maxid = UserImage.objects.filter(username=username).count()
        print(maxid)
        for num in range(1,int(maxid)+1):    
            quantity = request.POST[str(num) + 'quantity']   
            id = request.POST[str(num) +'id']
            s = UserImage.objects.filter(id=id).first()
            s.quantity = quantity
            s.save()
    images = UserImage.objects.filter(username=username)
    context = {'images':images,
...
def pl_adjust(request):
    if (request.method=='POST'):
        ...
        JidoPL2= request.POST['JidoPL2']
        NonTagPL2= request.POST['NonTagPL2']
        companyCode=user.profile.companycode
              
        ny = ZaikoTable.objects.get(companyCode=companyCode)
        ny.JidoTagPL=int(JidoPL2)
        ny.NonTagPL=int(NonTagPL2)
        ny.save()
        ...
        if (int(JidoPL)!=int(JidoPL2)):
            PLtype='1'
            PLname='自動倉庫PL_タグあり'
            PLvolume=str(int(NonTagPL2)-int(NonTagPL))
            savedata = Pallets.objects.create(
                    keyid = keyid,
                    PLtype = PLtype,
                    PLname = PLname,
        ...
        if (int(NonTagPL)!=int(NonTagPL2)):
            PLtype='2'
            PLname='タグなしパレット'
            PLvolume=str(int(JidoPL2)-int(JidoPL))
            savedata = Pallets.objects.create(
                    keyid = keyid,
                    PLtype = PLtype,
                    PLname = PLname,
...
def showpl(request):
    ...
    #context = {'images':images}
    return render(request, 'showpl.html')
